...
namespace Interface
{
    ...
    void DisplayParty(SDL_Renderer *Renderer, Party::Base &Party, ScreenDimensions &Screen)
    {
        ...
        for (auto i = 0; i < Party.Members.size(); i++)
        {
            ...
            Graphics::PutText(Renderer, ("RNK: " + std::to_string(Party.Members[i].Rank)).c_str(), Fonts::Normal, 0, clrWH, intBK, TTF_STYLE_NORMAL, Screen.InfoBoxX + 5 * Screen.IconSize + 2 * text_space, Screen.InfoBoxY + i * (Screen.IconSize + FontSize) + 2 * FontSize + text_space);
        }
